...
public class GaussianFitter3D extends IsoGaussianFitter {
    ...
    public double evaluate(double[] p, double x, double y, double z) {
        ...
        /*return p[0] + p[4] * net.calm.iaclasslibrary.Math.exp(-(net.calm.iaclasslibrary.Math.pow(x - p[5], 2.0) / (2.0 * p[1] * p[1])
        + net.calm.iaclasslibrary.Math.pow(y - p[6], 2.0) / (2.0 * p[2] * p[2])
        + net.calm.iaclasslibrary.Math.pow(x - p[7], 2.0) / (2.0 * p[3] * p[3])));*/
